...
class QuestionModelTests(TestCase):
    def test_was_published_recently_woth_future_question(self):
        time = timezone.now() + datetime.timedelta(days=30)
        future_question = Question(pub_date=time)
        self.assertIs(future_question.was_published_recently(), False)

    def test_was_published_rexently_with_old_questions(self):
        time= timezone.now() - datetime.timedelta(days=1, seconds=1)
        old_question = Question(pub_date=time)
        self.assertIs(old_question.was_published_recently(), False)

    def test_was_publishe_recently_with_recent_question(self):
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_two_past_questions(self):
        question1 = create_question(question_test="past question 1", days=-30)
        question2 = create_question(question_test="past question 2", days=-5)
        response = self.client.get(reverse("polls:index"))
